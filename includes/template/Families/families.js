...
(function ($) {
  ...
  function showFieldError(fieldSelector, message) {
    ...
    var $error = $(
      '<span class="error-message" style="color: #d63384; font-size: 12px; display: block; margin-top: 5px;">' +
        message +
        "</span>"
    );
    $field.after($error);
      ...
      success: function (response) {
        if (response.success) {
          if (response.data.available) {
            ...
            $status.html(
              '<span style="color: #d63384;">✗ Already exists</span>'
            );
          }
        } else {
  ...
  window.findPerson = function (field, gender) {
    ...
    $.ajax({
      ...
      error: function () {
        $content.html(
          '<div style="text-align: center; padding: 40px; color: #d63384;">Error loading person finder</div>'
        );
      },
    });
  };
  ...
  window.selectPerson = function (personID, personName) {
    ...
    $("#" + HP_Families.currentPersonField + "_display").val(
      personName + " - " + personID
    );
    closePersonFinder();
  };
  ...
  window.createPerson = function (field, gender) {
    ...
    if ($("#edit-family-form").length) {
      returnPage +=
        "&tab=edit&familyID=" +
        encodeURIComponent($("#original_familyID").val()) +
        "&tree=" +
        encodeURIComponent(tree);
    }
    var url =
      "admin.php?page=heritagepress-people&tab=add&tree=" +
      encodeURIComponent(tree) +
      "&gender=" +
  ...
  window.editPerson = function (field) {
    ...
    var url =
      "admin.php?page=heritagepress-people&tab=edit&personID=" +
  ...
  window.findPlace = function (field) {
    ...
    $.ajax({
      ...
      error: function () {
        $content.html(
          '<div style="text-align: center; padding: 40px; color: #d63384;">Error loading place finder</div>'
        );
      },
    });
  };
  ...
  window.findSource = function (field) {
    ...
    $.ajax({
      ...
      error: function () {
        $content.html(
          '<div style="text-align: center; padding: 40px; color: #d63384;">Error loading source finder</div>'
        );
      },
    });
  };
  ...
  window.findFamily = function (field) {
    ...
    $.ajax({
      ...
      error: function () {
        $content.html(
          '<div style="text-align: center; padding: 40px; color: #d63384;">Error loading family finder</div>'
        );
      },
    });
  };
