...
/**
 * Enhanced GEDCOM Parser Alternative for HeritagePress
 *
...
class HP_Enhanced_GEDCOM_Parser_Alt
{
  private $file_handle;
  private $tree_id;
  private $stats;
  ...
  /**
   * Parse a GEDCOM line (Enhanced style)
   */
  private function get_line()
  {
    ...
    if ($line = ltrim(fgets($this->file_handle, 1024))) {
      // Clean up the line (Enhanced style)
      $patterns = array('/��.*��/', '/��.*/', '/.*��/', '/@@/');
      $replacements = array('', '', '', '@');
  ...
  /**
   * Parse NAME field (Enhanced style)
  ...
  private function parse_source($source_id)
  {
    ...
    }    // Insert into sources table (handle duplicates properly)
    $table_name = $wpdb->prefix . 'hp_sources';

    // Check if source already exists
    $existing = $wpdb->get_var($wpdb->prepare(
      "SELECT COUNT(*) FROM $table_name WHERE sourceID = %s AND gedcom = %s",
    ...
    if ($existing) {
      ...
      switch ($this->save_state['del']) {
        ...
        case 'yes':
          ...
          $wpdb->update($table_name, $source_data, array(
            'sourceID' => $source_data['sourceID'],
            'gedcom' => $source_data['gedcom']
          ));
          break;
        case 'append':
  ...
  private function parse_repository($repo_id)
  {
    ...
    }    // Insert into repositories table (handle duplicates properly)
    $table_name = $wpdb->prefix . 'hp_repositories';

    // Check if repository already exists
    $existing = $wpdb->get_var($wpdb->prepare(
      "SELECT COUNT(*) FROM $table_name WHERE repoID = %s AND gedcom = %s",
      $repo_data['repoID'],
      $repo_data['gedcom']
    ));
    ...
    if ($existing) {
      ...
      switch ($this->save_state['del']) {
        ...
        case 'yes':
          // Update existing record
          $wpdb->update($table_name, $repo_data, array(
   ...
   */  private function save_individual($person_id, $info)
  {
    ...
    $table_name = $wpdb->prefix . 'hp_people';    // Apply offset for append mode
