...
<div class="wrap">
  <h1 class="wp-heading-inline">
    <?php _e('Cemetery Management', 'heritagepress'); ?> &gt;&gt; <?php _e('Add New Cemetery', 'heritagepress'); ?>
  </h1>
  ...
  <form method="post" enctype="multipart/form-data" id="cemetery-form">
    ...
    <input type="hidden" name="hp_cemetery_action" value="add">
    ...
    <table class="form-table">
      <tbody>
        <tr>
          ...
          <td>
            <input type="text" name="cemname" id="cemname" class="regular-text" required>
        ...
        <tr>
          <th scope="row">
            <label for="newfile"><?php _e('Map File to Upload', 'heritagepress'); ?></label>
          </th>
          <td>
            <input type="file" name="newfile" id="newfile" accept=".jpg,.jpeg,.png,.gif,.pdf">
            <p class="description">
              <?php _e('Upload a cemetery map, plot plan, or related image. Allowed formats: JPG, PNG, GIF, PDF', 'heritagepress'); ?>
            </p>
          </td>
        </tr>
        ...
        <tr>
          <th scope="row">
            <label for="maplink"><?php _e('Map File Path', 'heritagepress'); ?></label>
          </th>
          <td>
            <input type="text" name="maplink" id="maplink" class="regular-text">
            <p class="description">
              <?php _e('Path to existing map file, or will be auto-populated when uploading a file above.', 'heritagepress'); ?>
            </p>
          </td>
        </tr>
        ...
        <tr>
          <th scope="row">
            <label for="place"><?php _e('Link to Place', 'heritagepress'); ?></label>
          </th>
          <td>
          ...
          <td>
            <div id="cemetery-map" style="height: 400px; width: 100%; margin-bottom: 10px; border: 1px solid #ccd0d4;">
              <p style="text-align: center; padding: 50px; color: #666;">
                ...
                <br><br>
                <?php _e('Users could click on the map to set coordinates, or enter them manually below.', 'heritagepress'); ?>
              </p>
            </div>
          </td>
        </tr>
    ...
    <p class="submit">
      <input type="submit" name="submit" class="button-primary" value="<?php _e('Add Cemetery', 'heritagepress'); ?>">
...
<script>
  jQuery(document).ready(function($) {
    ...
    $('#cemetery-form').submit(function(e) {
      ...
      // Validate file upload and map link
      var hasFile = $('#newfile')[0].files.length > 0;
      var hasMapLink = $('#maplink').val().trim();

      if (hasFile && !hasMapLink) {
        alert('<?php _e('Please specify a map file path.', 'heritagepress'); ?>');
        $('#maplink').focus();
        e.preventDefault();
        return false;
      }
